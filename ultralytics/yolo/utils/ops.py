...
def scale_xy(img1_shape, xy, img0_shape, ratio_pad=None, num_point=17):
    if ratio_pad is None:  # calculate from img0_shape
        gain = min(img1_shape[0] / img0_shape[0], img1_shape[1] / img0_shape[1])  # gain  = old / new
        pad = (img1_shape[1] - img0_shape[1] * gain) / 2, (img1_shape[0] - img0_shape[0] * gain) / 2  # wh padding
    else:
    ...
    for i in range(num_point):
        xy[..., i * 3] -= pad[0]  # x padding
        xy[..., 1 + i * 3] -= pad[1]  # y padding
        xy[..., i * 3] /= gain
        xy[..., 1 + i * 3] /= gain
        xy[..., 2 + i * 3] = torch.sigmoid(xy[..., 2 + i * 3])
    return xy


def line_keypoints(ori_img, kpts, color=(0, 0, 255)):
    cv2.line(ori_img, (int(kpts[0]), int(kpts[1])), (int(kpts[3]), int(kpts[4])), color, 2)
    cv2.line(ori_img, (int(kpts[0]), int(kpts[1])), (int(kpts[6]), int(kpts[7])), color, 2)
    cv2.line(ori_img, (int(kpts[3]), int(kpts[4])), (int(kpts[9]), int(kpts[10])), color, 2)
    cv2.line(ori_img, (int(kpts[6]), int(kpts[7])), (int(kpts[12]), int(kpts[13])), color, 2)
    cv2.line(ori_img, (int(kpts[9]), int(kpts[10])), (int(kpts[15]), int(kpts[16])), color, 2)
    cv2.line(ori_img, (int(kpts[12]), int(kpts[13])), (int(kpts[18]), int(kpts[19])), color, 2)
    cv2.line(ori_img, (int(kpts[30]), int(kpts[31])), (int(kpts[24]), int(kpts[25])), color, 2)
    cv2.line(ori_img, (int(kpts[24]), int(kpts[25])), (int(kpts[18]), int(kpts[19])), color, 2)
    cv2.line(ori_img, (int(kpts[18]), int(kpts[19])), (int(kpts[15]), int(kpts[16])), color, 2)
    cv2.line(ori_img, (int(kpts[15]), int(kpts[16])), (int(kpts[21]), int(kpts[22])), color, 2)
    cv2.line(ori_img, (int(kpts[21]), int(kpts[22])), (int(kpts[27]), int(kpts[28])), color, 2)
    cv2.line(ori_img, (int(kpts[36]), int(kpts[37])), (int(kpts[33]), int(kpts[34])), color, 2)
    cv2.line(ori_img, (int(kpts[18]), int(kpts[19])), (int(kpts[36]), int(kpts[37])), color, 2)
    cv2.line(ori_img, (int(kpts[36]), int(kpts[37])), (int(kpts[42]), int(kpts[43])), color, 2)
    cv2.line(ori_img, (int(kpts[42]), int(kpts[43])), (int(kpts[48]), int(kpts[49])), color, 2)
    cv2.line(ori_img, (int(kpts[15]), int(kpts[16])), (int(kpts[33]), int(kpts[34])), color, 2)
    cv2.line(ori_img, (int(kpts[33]), int(kpts[34])), (int(kpts[39]), int(kpts[40])), color, 2)
    cv2.line(ori_img, (int(kpts[39]), int(kpts[40])), (int(kpts[45]), int(kpts[46])), color, 2)
    return ori_img
...
def non_max_suppression(
    ...
    t = time.time()
    output = [torch.zeros((0, 57 + nm), device=prediction.device)] * bs
    prediction = torch.cat((prediction, pre_kpts), dim=1)
